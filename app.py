...
st.set_page_config(
    page_title="Data Warehouse Meta Sports", page_icon="🏃‍♂️", layout="wide"
)
...
if 'Año' in df_all.columns and df_all['Año'].notna().any():
  años_disponibles = sorted(
      [int(a) for a in df_all['Año'].dropna().unique()], reverse=True
  )
...
st.title('🏃‍♂️ Data Warehouse Meta Sports')
st.subheader('📊 Panel Consolidado de Inscripciones')
...
operadores_unicos = (
    df_filtrado[df_filtrado['Operador'] != 'Sin Operador / Directo'][
        'Operador'
    ].nunique()
...
distancias_unicas = (
    df_filtrado['Distancia'].nunique()
    if 'Distancia' in df_filtrado.columns
    else 0
)
eventos_activos = (
    df_filtrado['Evento'].nunique() if 'Evento' in df_filtrado.columns else 0
)
...
with tab3:
  ...
  if (
      'Fecha_Inscripcion' in df_filtrado.columns
      and df_filtrado['Fecha_Inscripcion'].notna().any()
  ):
    df_filtrado['Fecha_DT'] = pd.to_datetime(
        df_filtrado['Fecha_Inscripcion'], errors='coerce'
    )
    df_fecha = (
        df_filtrado.dropna(subset=['Fecha_DT'])
        .groupby(df_filtrado['Fecha_DT'].dt.date)
        .size()
...
with tab4:
  ...
  cols_mostrar = [
      ...
      if c in df_exploracion.columns
  ]
